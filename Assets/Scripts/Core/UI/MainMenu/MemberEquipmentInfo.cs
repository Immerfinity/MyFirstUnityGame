...
public class MemberEquipmentInfo : MonoBehaviour
{
    ...
    public void SetPartyMember(PartyMember member)
    {
        ...
        weaponText.text = equippedWeapon is not null ? $"Weapon: {equippedWeapon.ItemName}" : "Weapon: Нет";
        armorText.text = equippedArmor is not null ? $"Armor: {equippedArmor.ItemName}" : "Armor Нет";
        accessoryText.text = equippedAccessory is not null ? $"Accessory: {equippedAccessory.ItemName}" : "Accessory: Нет";
    }
